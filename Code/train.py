...
def train(rank, params, shared_model, counter, lock, optimizer=None):
    ...
    while True: # repeat
        # incrementing the episode length by one
        ...
        if done: # if it is the first iteration of the while loop or if the game was just done, then:
            cx =torch.zeros(1, 256) # the cell states of the LSTM are reinitialized to zero
            hx = torch.zeros(1, 256) # the hidden states of the LSTM are reinitialized to zero
        else: # else:
            cx = cx.detach()
            hx = hx.detach()# we keep the old hidden states, making sure they are in a torch variable
        ...
        for step in range(params.num_steps):
            episode_length += 1# going through the num_steps exploration steps
            ...
            action = prob.multinomial(num_samples=1).data # selecting an action by taking a random draw from the prob distribution
        ...
        if not done: # if we are not done:
            ...
            R = value.detach() # we initialize the cumulative reward with the value of the last shared state
        ...
        # making sure the cumulative reward R is a torch Variable
